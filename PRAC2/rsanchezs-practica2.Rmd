...
output:
  ...
  html_document:
    ...
    toc_depth: 2
    includes:
      in_header: M2.951-PRAC2-header.html
  pdf_document:
    highlight: zenburn
    toc: yes
---
...
* Aquest document en formats __pdf__ i __docx__ amb el nom rsanchezs_practica2.
...
Gràcies a l´anterior canvi de tipus podem calcular la <a name="edat">edat</a> del client i crear un nova columna que anomenarem `EDAT` amb el valor calculat de la següent manera:
...
En un altre ordre de coses, crearem un nou conjunt de dades que es tractarà de la consulta __left-join__ de les taules `client` i `ticket` que estan relacionades a la fase de dades per la clau forana `CODCLIENT`:
...
A continuació, crearem una nova columna amb la suma dels imports totals dels tickets per a cada client. Es a dir, agruparem les observacions per client i calcularem una nova variable amg la funció d´agregació `sum()`:
...
Amb la intenció de simplificar el nostre model, reduirem el nombre de classes i agruparem `Food`, `Catering` i `Alimentación` en una sola variable atès que sembla
...
En primer lloc, calculem un resúm estadístic de la variable `TOTAL` per tal de coneixer la mitjana de compres dels nostres clients:
...
La funció `createDataPartition` ens permetrà crear els subconjunts de dades. 
...
A continuació podem comprovar que hem obtingut un conjunt de entrenament de 2467
observacions:
